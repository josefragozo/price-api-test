...
public class PriceRepositoryImpl implements PriceRepository {
    ...
    public Price getPriceByBrandIdAndProductIdAndDate(
        ...
        final var prices = priceJpaRepository.getByBrandIdAndProductId(brandId, productId);

        final var priceMaxPriority = prices.stream()
            .filter(f ->
                (
                    date.isAfter(f.getStartDate()) && date.isBefore(f.getEndDate())
                )
                    || date.isEqual(f.getStartDate())
                    || date.isEqual(f.getEndDate())
            ).max(Comparator.comparingInt(PriceEntity::getPriority));

        final var productPrice = priceMaxPriority.orElseThrow(PriceNotFoundException::new);

        return priceOf(productPrice);
    }
